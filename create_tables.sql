...
BEGIN
  ...
  IF found_unit > 0 THEN
    SELECT measureunitcode INTO tmp_unit FROM sample_parameters
      WHERE sample_parameters.param_cd = NEW.param_cd;	
    -- check that analysis_unit equals measureunitcode
	  IF NEW.analysis_unit != tmp_unit THEN
	    RAISE EXCEPTION 'Units not equal. Convert prior to insterting.';
	  END IF;
  END IF;
RETURN NEW;  
END;
